...
(`resoluciones/backend`) sólo guarda/lista/entrega páginas.

## Correrlo

```bash
cd resoluciones/frontend
npm install
copy .env.example .env      # VITE_API_URL -> backend; VITE_OCR_API_URL -> OCR GAMC
npm run dev                  # http://localhost:5173
```

Necesita el backend (`resoluciones/backend`, puerto 8080) corriendo, y un
